...
<%@page import="com.ytoxl.module.uhome.uhomebase.dataobject.BrandSort"%>
...
<%@page import="com.ytoxl.module.uhome.uhomebase.service.BrandSortService"%>
<%@page import="com.ytoxl.module.uhome.uhomecontent.service.NoticeService"%>
...
<body>
	<%
	    ...
	    BrandSortService brandSortService = (BrandSortService)(SpringContextUtils.getContext().getBean("brandSortServiceImpl"));
	    ...
	    NoticeService noticeService = (NoticeService)(SpringContextUtils.getContext().getBean("noticeServiceImpl"));
	  	//首页顶部广告
		Advertisement homeTopAdv = advService.getHomeTopAdv();
		request.setAttribute("homeTopAdv",homeTopAdv);
	%>
	<!--顶部广告-->
    ...
    <%
		...
		List<Product> mostHits = productService.getProductListByHits(8);
		//往期特卖列表
		List<Advertisement> advs = advService.listHomeHistorySpecialAdv();
		//热销品牌
		List<Brand> hotBrands = brandService.listHotBrands();
		//首页品牌集中营
		/* List<Brand> brands = brandService.listBrands(); */
		//品牌集中营优化排序
		List<BrandSort> brands = brandSortService.getBrandListGroupByType();
		//右部5个广告位置
		List<Advertisement> fiveAdvs = advService.listHomeRightAdvertisement();
		//公告区
		List<Notice> notices = noticeService.searchIndexNotice();
		//首页轮播广告
		List<Advertisement> broadcastAdvs = advService.listHomeBroadcastAdv();
		...
		request.setAttribute("hotBrands",hotBrands);
		request.setAttribute("brands",brands);
		request.setAttribute("fiveAdvs",fiveAdvs);
		if(notices != null && notices.size() == 0){
			notices = null;
		}
		request.setAttribute("notices",notices);
		request.setAttribute("broadcastAdvs",broadcastAdvs);
		int broadcastAdvsRank = 1;
    %>
    <fmt:setLocale value="zh_CN"/>
    <!--主体 start-->
	<div class="wrap">	
		<c:if test="${fn:length(broadcastAdvs) > 0}">
			<!--首页焦点图 start-->
			<div class="slider m-mt10p" id="j_slider">
		        <ul class="slider-img" id="j_img">
		        	<c:forEach items="${broadcastAdvs }" var="broadcastAdvs">
						<% if(broadcastAdvsRank == 1){ %>
							<c:if test="${broadcastAdvs.isLogin==null || broadcastAdvs.isLogin==0 }">
		        				<li style="display:list-item;">
									<c:if test="${broadcastAdvs.linkUrl != null && broadcastAdvs.linkUrl != '' }">
		        						<a target="_blank" href="${broadcastAdvs.linkUrl }" >
		        							<img alt="${broadcastAdvs.name }" src="<zx:thumbImage originalPath='${broadcastAdvs.imageUrl}' imageType="t32"></zx:thumbImage>" width="960" height="280">
		        						</a>
		        					</c:if>
									<c:if test="${broadcastAdvs.linkUrl == null || broadcastAdvs.linkUrl == '' }">
		        						<a  href="#" >
							...
							<c:if test="${broadcastAdvs.isLogin == 1 }">
								<li style="display:list-item;">
									<c:if test="${broadcastAdvs.linkUrl != null && broadcastAdvs.linkUrl != '' }">
		        						<a  href="javascript:void(0);" onclick="checkLogin('${broadcastAdvs.linkUrl}','${broadcastAdvs.advertisementId}','<%=CodeConstants.UNLOGIN_ERROR %>','<%=CodeConstants.LOGIN_TIMEOUT_ERROR %>')">
		        							<img alt="${broadcastAdvs.name }" src="<zx:thumbImage originalPath='${broadcastAdvs.imageUrl}' imageType="t32"></zx:thumbImage>" width="960" height="280">
		        						</a>
		        					</c:if>
									<c:if test="${broadcastAdvs.linkUrl == null || broadcastAdvs.linkUrl == '' }">
		        						<a  href="javascript:void(0);" onclick="checkLogin('#','${broadcastAdvs.advertisementId}','<%=CodeConstants.UNLOGIN_ERROR %>','<%=CodeConstants.LOGIN_TIMEOUT_ERROR %>')">
		        							<img alt="${broadcastAdvs.name }" src="<zx:thumbImage originalPath='${broadcastAdvs.imageUrl}' imageType="t32"></zx:thumbImage>" width="960" height="280">
		        						</a>
		        					</c:if> 
								</li>
								<input type="hidden" id="${broadcastAdvs.advertisementId}" />
							</c:if>
		        		<% broadcastAdvsRank = 2;}else{  %>	
		        				<c:if test="${broadcastAdvs.isLogin==null || broadcastAdvs.isLogin==0 }">
		        				<li >
									<c:if test="${broadcastAdvs.linkUrl != null && broadcastAdvs.linkUrl != '' }">
		        						<a target="_blank" href="${broadcastAdvs.linkUrl }" >
		        ...
		        <ul class="slider-trigger" id="j_strigger">
		       		<c:forEach items="${broadcastAdvs }" var="broadcastAdvs">
		        		<% if(broadcastAdvsRank == 2){ %>
		        				<li class="on">•</li>
		        		<% broadcastAdvsRank = 3;}else { %>	
		        				<li>•</li>
		        		<%} %>
					</c:forEach>
		        </ul>
		...
		<div class="mian-block cf">
			...
			<div class="notice">
				...
				<div class="right-block-t">
					<a href="${_ctxPath }/notice/index-1.htm" target="_blank">更多></a>
					<b>公告区 </b>
				</div>
				<div class="block-gg">
					<ul>
						<c:forEach items="${notices}" var="notices" varStatus="status">
							<li><a href="${_ctxPath }/notice/detail-${notices.noticeId}.htm" title="${notices.title}" target="_blank">${notices.title}</a></li>
						</c:forEach>
					</ul>
				</div>
				</c:if>
				...
				<div class="cl"></div>
	      		...
	      		<!--热销品牌 end-->
	      		<!--人气商品 start-->
	      		<cache:cache time="${_cacheSec}" refresh="${_needRefresh}" key="index-1-popularProduct">
				<div class="right-block-t">
					<b>人气商品</b>
				</div>
		...
		<cache:cache time="${_cacheSec}" refresh="${_needRefresh}" key="brands">
	            ...
	            <div class="upcom-pro-tit">
	                ...
	                <span class="tab-ctrl"><i class="cur" id="1">服装箱包</i>|<i id="2">母婴用品</i>|<i id="3">家具用品</i>|<i id="4">美容护肤</i></span>
	            </div>
    ...
    <%--
    //保存品牌预约  
    brandOpenInform(brandId,obj,<%=MailSubscribe.TYPE_BRAND_SUBSCRIBE%>,<%=MailSubscribe.STATUS_SUBSCRIBE%>);
    --%>
    //邮件订阅
    subEmail("#postSubscriptionEmail",<%=MailSubscribe.TYPE_SPECIAL_SELLER%>,<%=MailSubscribe.STATUS_SUBSCRIBE%>)
    <%--
    //即将上线订阅
    postSubSoonEmail(brandId,planId,obj,<%=MailSubscribe.TYPE_TOBEONTHELINE%>,<%=MailSubscribe.STATUS_SUBSCRIBE%>)
    --%>
