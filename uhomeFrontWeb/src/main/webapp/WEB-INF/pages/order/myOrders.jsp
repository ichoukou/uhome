...
<body>
    ...
    <div class="m-w960p cf">
        ...
        <div class="inf-detail inf-mine m-mt20p">
            <form method="post" action="${_ctxPath}/user/orders.htm" id="searchForm">
                    		...
                    		<tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td valign="top">
                                            ...
                                            <c:if test="${order.status == 1}">
                                            	<div><a href="javascript:;" value="${order.orderId}" class="normal payment">立即付款</a></div>
			                    ...
			                    <c:if test="${null == order.parentOrderId && null == order.status}">
                                    ...
                                    <tr>
                                        <td valign="top">
	                                        <span>
	                                        <label><input disabled="disabled" type="checkbox" class="ord-checkbox"  status-value="${order.status}" data-value="${order.orderId}">
	                                        ${order2.orderNo}</label>
	                                        </span>
                                        </td>
                                        <td class="ord-sales">
                                            <ul>
                                        ...
                                        <td valign="top">${order2.address.receiverName}</td>
                                        <td valign="top">
                                            <div class="cor-red">¥${order2.payment.paymentAmount}元</div>
                                            <div class="cor-red">(含运费${order2.orderPostage.postage}元)</div>
                                        ...
                                        <td valign="top" rowspan="${fn:length(order.childOrders)}">
                                            <div><a target="_blank" href="${_ctxPath}/order/order-getOrderCoupons.htm?orderIds=${order.childOrderIds}" class="high-light">查看</a></div>
                                            <c:if test="${order2.status == 1}">
                                            	<div><a href="javascript:;" value="${order.childOrderIds}" class="normal corOrangePayment">立即付款</a></div>
                                            	<div><a href="javascript:;" value="${order.childOrderIds}" class="normal corOrangeCancel">取消</a></div>
                                            </c:if>
                                        </td>
                                        </c:if>
                                    </tr>
    ...
    <script type="text/javascript">
    	//使用优惠券付款连接
    	$(".normal.corOrangePayment").on("click",function(event){
    		var $this = $(this);
    	...
    	//未使用优惠券付款连接
    	$(".normal.payment").on("click",function(event){
    		var $this = $(this);
    	...
    	//取消使用优惠券的订单
    	$(".normal.corOrangeCancel").on("click",function(event){
    		 var $this = $(this);
    		 var orderIds = $this.attr("value");
 	    	 $.dialog({
                 title: false,
