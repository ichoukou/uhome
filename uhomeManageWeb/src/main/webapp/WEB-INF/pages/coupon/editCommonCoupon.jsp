...
<head>
	...
	<script type="text/javascript" src="${_jsPath }/pages/coupon/editCommonCoupon.js"></script>
	<script type="text/javascript" src="${_jsPath }/pages/coupon/validator.js"></script>
	<script type="text/javascript" src="${_jsPath }/pages/coupon/tree.js"></script>
</head>
<!--start header-->
...
<div class="body m-w980p">
    ...
    <ul class="m-clear">
      ...
      <li><a class="current-chose" href="${_ctxPath}/admin/coupon/coupon-editCommonCoupon.htm">配置优惠券</a></li>
      </aut:authorize>
...
<form id="addForm">
	<input type="hidden" name="event.type" value="<%=Event.TYPE_COMMON%>"/>
	...
	<table id="tab" class="tab-bor" width="100%">
		...
		<tbody>
			<tr class="list-tr">
				<td><input type="text" class="j-edited input-nobor" name="event.eventName" /></td>
				<td><input type="text" class="j-edited input-nobor allowance" name="event.couponConfigList[0].allowance"  onblur="checkAllowance(this)"/></td>
				<td><input type="text" class="j-edited input-nobor limitCharge" name="event.couponConfigList[0].limitCharge"  onblur="checkDouble(this)"/></td>
				<td>
					<input value="" class="j-edited Wdate" type="text" name="event.couponConfigList[0].startTime" 
						id="startTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d',maxDate:'#F{$dp.$D(\'endTime\')}',readOnly:true,onpicked:pickedFunc})" /> 
					至
					<input value="" class="j-edited Wdate" type="text" name="event.couponConfigList[0].endTime" 
						id="endTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startTime\')||\'%y-%M-%d\'}',readOnly:true,onpicked:pickedFunc})" />
				</td>
				<td><a href="javascript:;" id="eventRange">点击选择</a></td>
				<td><input type="text" class="j-edited input-nobor" name="event.couponConfigList[0].couponCount"  onblur="checkCouponCount(this)"/></td>
			</tr>
		</tbody>
	</table>
	<div class="m-mt10p">
...
<div class="order-management">
	...
	<c:set var="notStart" value="<%=Event.ACTIVITYSTATUS_NOT_START %>"></c:set>
	...
	<form>
		...
		<table id="tab" class="tab-bor" width="100%">
			...
			<tbody>
				<tr class="list-tr">
					<c:set var="couponConfig" value="${event.couponConfigList[0]}"></c:set>
					<td>
					...
					<td>
						...
						<input type="text" class='j-edited input-nobor m-hide allowance' name="event.couponConfigList[0].allowance" value="${couponConfig.allowance }"/>
						<input type="hidden" name="event.couponConfigList[0].couponConfigId" value="${couponConfig.couponConfigId }"/>   
					</td>
					<td>
						<span class="j-normal">${couponConfig.limitCharge }</span>
						<input type="text" class='j-edited input-nobor m-hide limitCharge' name="event.couponConfigList[0].limitCharge" value="${couponConfig.limitCharge }" />
					</td>
					<td>
						...
						<input type="text" class='j-edited Wdate m-hide' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true,minDate:'%y-%M-%d'})" 
							name="event.couponConfigList[0].startTime" value="<fmt:formatDate value='${couponConfig.startTime }' pattern='yyyy-MM-dd'/>" />
						至
						<label class="j-normal"><fmt:formatDate value="${couponConfig.endTime }" pattern="yyyy-MM-dd"/></label>
						<input type="text" class='j-edited Wdate m-hide' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true,minDate:'%y-%M-%d'})" 
							name="event.couponConfigList[0].endTime" value="<fmt:formatDate value='${couponConfig.endTime }' pattern='yyyy-MM-dd'/>" />
					</td>
					<td><p>${event.eventRangeNames }</p></td>
					<td>
						<span class="j-normal">${couponConfig.couponCount }</span>
						<input type="text" class='j-edited input-nobor m-hide' name="event.couponConfigList[0].couponCount" value="${couponConfig.couponCount }" />
					</td>
					<td>
					...
					<td>
						...
						<c:if test="${event.activityStatus != expired }">
							...
							<a href="javascript:void(0);" class="j-export">导出</a>
...
<script type="text/javascript">
	var allPlan = <%=com.ytoxl.module.uhome.uhomebase.dataobject.EventRange.TYPE_ALLPLAN%>;
</script>
</body>
</html>