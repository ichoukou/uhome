			      ...
			      <c:forEach items="${planCheckPage.result }" var="planCheck" varStatus="status">
  					...
  					<tr>
 						<c:choose>
			      		...
			      		<td><fmt:formatDate value='${planCheck.oneFeedbackTime}' pattern='yyyy.MM.dd HH:mm:ss'/></td>
			      		<td>一次对账结束</td>
			      		<td><c:choose>   
			      			 <c:when test="${planCheck.feedbackCount==0}"></c:when>     
			      			 <c:when test="${planCheck.feedbackStatus==1}">已完成 </c:when>  
			      			  <c:otherwise>
			      			  	<aut:authorize url="/admin/planCheck/planCheck-finishTwoCheck">
			      			  	 <input type="button" onclick="javascript:window.location.href='${_ctxPath}/admin/planCheck/planCheck-finishTwoCheck.htm?planCheck.planCheckId=${planCheck.planCheckId}';" class="m-btn-done" value="完成" />  
			      				</aut:authorize>
			      			  </c:otherwise>
			      			</c:choose> 
			      		</td>
