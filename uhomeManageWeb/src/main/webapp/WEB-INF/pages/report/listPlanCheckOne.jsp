			...
			<form class="m-clear m-mt10p" action="${_ctxPath}/admin/planCheck/planCheck-listPlanCheck.htm" class="m-clear m-mt10p" id="searchForm"  name="searchForm" method="post"  >
				...
				<select name="planCheckPage.params.planId" id="planId" class="m-sel">
					...
					<select name="planCheckPage.params.sellerId" id="sellerId" class="m-sel">
					  ...
					  <input name="planCheckPage.params.name" id="name" type="text" value="${planCheckPage.params.name }" class="txt-input input-marks" data-default="输入活动名称，商家名称进行搜索"/>
			      ...
			      <c:forEach items="${planCheckPage.result }" var="planCheck" varStatus="status">
  					<tr>
 						<c:choose>
			      		...
			      		<td><fmt:formatDate value='${planCheck.createTime}' pattern='yyyy.MM.dd HH:mm:ss'/></td>
			      		<td><c:choose>   
			      			 <c:when test="${planCheck.feedbackCount== null}">未申请   </c:when>     
			      			 <c:otherwise>  已申请   </c:otherwise>  
			      			</c:choose> 
			      		</td>
			      		<td><c:choose>   
			      			 <c:when test="${planCheck.feedbackCount==null}"></c:when>     
			      			 <c:when test="${planCheck.feedbackStatus==1}">已完成 </c:when>  
			      			 <c:otherwise>
			      			  <aut:authorize url="/admin/planCheck/planCheck-finishOneCheck">
			      			 	<input type="button" class="m-btn-done" value="完成" onclick="javascript:window.location.href='${_ctxPath}/admin/planCheck/planCheck-finishOneCheck.htm?planCheck.planCheckId=${planCheck.planCheckId}';" />
			      			 </aut:authorize>
			      			 </c:otherwise>  
			      			</c:choose> 
			      		</td>
